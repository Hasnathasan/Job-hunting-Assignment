  ...
  return (
    <div className="border border-slate-200 p-9 rounded-lg">
      ...
      <div className="my-5">
        {jobtype.map((job) => (
          <span
            ...
            className="px-2 mr-2 py-1 text-base text-violet-500 font-bold border border-violet-400 rounded"
          >
            {job}
          </span>
        ))}
      ...
      <Link to={`/jobDetails/${id}`}>
        <button className="btn border-none text-lg capitalize rounded text-white mt-5">
          View Details
        </button>
      </Link>
    </div>
